...
A screen recording of the working implementation — browsing the competition, the live
countdown, registering (watch the spot counter change), the ENG/हिंदी toggle, and
...
> If the player does not load inline, the file is in the repo at
> [`mobile/assets/screen_rec.mp4`](mobile/assets/screen_rec.mp4).
...
6. [How the screen stays dynamic](#how-the-screen-stays-dynamic)
7. [Concurrency and data consistency](#concurrency-and-data-consistency)
8. [Validations and edge cases](#validations-and-edge-cases)
...
| --- | --- |
| `feedants-classical-dance` | The design screen: registration open, live countdown, spots filling |
...
| Email | State |
| --- | --- |
| `demo@feedants.com` | Not registered — shows the `Register Now` path end to end |
| `guest@feedants.com` | Not registered — a second account for testing concurrent booking |
...
`npm run dev` / `npm start` runs `src/seed/autoSeed.js` before the server accepts traffic. On
every boot it re-anchors all competition dates to the current time, clears registrations for the
main competition and resets its spot counter to zero.

This exists so the project is reviewable at any point in the future: the registration countdown
always reads roughly `01d : 06h` rather than a deadline that silently expired months ago, and a
reviewer can always register from a clean slate and watch `20 → 19 spots left` update live.
The trade-off is that a restart discards registration state — correct for a demo, and exactly
what you would not want in production.
